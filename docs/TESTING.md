...
建议 **按 A → F 顺序** 执行；每步「预期结果」均满足后再进行下一步。
...
- 本项目 `status.py` 目前 **仅账户+持仓**，不含订单列表（后续可加）
...
1. 在 [TASKS.md](../TASKS.md) 勾选你完成的 M3 项（若做了 Phase D/E）
2. 把异常日志保存，方便 Agent 排查
3. 下一步：订单查询 CLI、或你的第一个自定义策略
