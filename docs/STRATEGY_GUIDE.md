...
- 策略实现：`src/strategies/`（`sma_crossover`、`ema_crossover`、`donchian_breakout`、`bollinger_rsi`、`momentum_rotation`）
- 共享指标：`src/strategies/indicators.py`
- 策略配置：`config/strategies/*.yaml`
- 策略框架：`src/strategy/base.py`、`src/strategy/registry.py`
...
已实现 **5 个** 日 K 策略，均注册于 `src/strategy/registry.py`，配置在 `config/strategies/`。

| 策略名 | 类型 | 核心逻辑 | 默认参数 | 配置文件 |
|--------|------|----------|----------|----------|
| `sma_crossover` | 趋势 | 快 SMA 上穿慢 SMA → BUY；下穿 → SELL | fast=10, slow=30 | `sma_crossover.yaml` |
| `ema_crossover` | 趋势 | 快 EMA 上穿慢 EMA → BUY；下穿 → SELL | fast=12, slow=26 | `ema_crossover.yaml` |
...
- 默认 watchlist：`HK.00700`、`HK.09988`、`HK.02318`、`HK.03690`（须与 `settings.yaml` 白名单一致，见 [WATCHLIST.md](WATCHLIST.md)）
- 每次信号默认 **100 股**（`qty` 可在 YAML 修改）
- 同一标的 **防重复信号**（与 SMA 相同 `_last_side` 机制）
- 仅输出 `Signal`，不直接下单；执行走模拟盘 + 风控
...
```bash
.venv/bin/python scripts/run_paper.py --strategy ema_crossover --mode daily --market HK --once
.venv/bin/python scripts/run_paper.py --strategy donchian_breakout --mode daily --market HK --once
.venv/bin/python scripts/run_paper.py --strategy bollinger_rsi --mode daily --market HK --once
.venv/bin/python scripts/run_paper.py --strategy momentum_rotation --mode daily --market HK --once
```

**`momentum_rotation` 说明：** 截面策略会在 watchlist **最后一只**标的的 K 线到达时统一排名并发出调仓信号；单标的 `--once` 跑一轮时，需 engine 按 watchlist 顺序喂齐所有 symbol 才会 rebalance。

---

## 3. 策略详解：SMA 均线交叉（`sma_crossover`）

### 3.1 策略思想（用 plain language）
...
### 3.3 当前配置（`config/strategies/sma_crossover.yaml`）

```yaml
name: sma_crossover
market: HK
symbols:
  - HK.00700          # 腾讯控股（可添加多只股票，见 3.7 节）
params:
...
**总结：** 第一梯队含 **5 个日 K 策略**（SMA / EMA 交叉、Donchian 突破、Bollinger+RSI 均值回归、动量轮动）。参数在 YAML，执行走 **模拟盘 + 风控**。换新策略时，用第 6 节的话术告诉 Agent，并强调 **BaseStrategy + Signal + 模拟盘** 即可。
